# Purpose: Fix the dots in users' emails in downloaded messages.
# Author : Hanwen (Steinway) Wu
# Origin : https://glot.io/snippets/efyndm7qs5
# Ref.   : https://github.com/icy/google-group-crawler/issues/16
...
class ImportScripts::Mbox < ImportScripts::Base
  ...
  BATCH_SIZE = 1000
  CATEGORY_ID = 6
  MBOX_DIR = File.expand_path("/tmp/google_group/")
  USERS = File.expand_path("/tmp/google_group/users.csv")
  ...
  def open_db
    puts "opening #{MBOX_DIR}/index.db"
  ...
  def load_users
    ...
    """
    users = []
    CSV.foreach(File.path("#{USERS}")) do |record|
      user = {}
      user["email"] = record[0]
      user["nickname"] = record[1]
      # user["joindate"] = DateTime.strptime("#{record[6]}-#{record[7]}-#{record[8]} #{record[12]}", "%Y-%m-%d %Z")
      users << user
    end
  ...
  def match_users(users, email)
    return email if email.nil?
    puts "finding users #{email.gsub(/\.+@/,".*@")}"
  ...
  def extract_name(mail)
    ...
    if mail.from.present?
      ...
      from_email.gsub!(/ at /, '@')
      from_email.gsub!(/ \(.*$/, '')
      search = match_users(@users, from_email)
    ...
    if display_names.present?
      ...
      from_name = search["nickname"] unless search.nil?
    end
